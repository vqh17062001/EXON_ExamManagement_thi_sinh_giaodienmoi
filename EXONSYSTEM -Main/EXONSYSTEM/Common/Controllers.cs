...
namespace EXONSYSTEM.Common
{
    public class Controllers
    {
        ...
        public void GenerateConfigFile(ConfigApplication ca, out ErrorController EC)
        {
            try
            {
                ...
                if (EC.ErrorCode == Constant.STATUS_OK)
                {
                    ...
                    EC = new ErrorController(Constant.STATUS_OK, "Tệp cấu hình được tạo tại " + Constant.PATH_CONFIG_FILE);
                }
            }
            catch (Exception ex)
            {
        ...
        public void GetConfigFile(out ConfigApplication rCA, out ErrorController EC)
        {
            try
            {
                if (File.Exists(Constant.PATH_CONFIG_FILE))
                {
                    ...
                    // Delete file tmp
                    File.Delete(Constant.PATH_CONFIG_FILE_TMP);
                    EC = new ErrorController(Constant.STATUS_OK, "Nhận tệp cấu hình thành công.");
                }
                else
                {
                    rCA = null;
                    EC = new ErrorController(Constant.STATUS_NORMAL, "Không tìm thấy tệp cấu hình.");
                }
            }
            catch (Exception ex)
            {
        ...
        public void ShowNotificationForm(int typeNotification, string content, Form formOwner)
        {
            ...
            else if (typeNotification == Constant.TYPE_NOTIFICATION_WARNINGOPENWORD)
            {
                DTO.NotificationForm.Header = Properties.Resources.MSG_MESS_0044;
                DTO.NotificationForm.ContentOpenWord = content;
                DTO.NotificationForm.TextMbtnOKOpenWord = Properties.Resources.MSG_MESS_0042;
                DTO.NotificationForm.TextMbtnCancel = Properties.Resources.MSG_MESS_0043;
            }
            DTO.NotificationForm.Owner = formOwner;
            DTO.NotificationForm.ShowDialog();
        }
        private SqlConnection Sql;
        public void ShowNotificationFormResult(string content, Form formOwner, int DivisionShiftID, int ContestantShiftID, SqlConnection sql)
        {
            Sql = sql;
            DTO.NotificationForm = new frmNotification(DivisionShiftID, ContestantShiftID);
            DTO.NotificationForm.Header = Properties.Resources.MSG_GUI_0050;
            DTO.NotificationForm.Content = content;
            DTO.NotificationForm.TextMbtnOK = Properties.Resources.MSG_GUI_0033;
            DTO.NotificationForm.SetMbtnOK(false);

            Timer timerGetStatusComplete = new Timer();
            timerGetStatusComplete.Interval = 1000;
            timerGetStatusComplete.Start();
            timerGetStatusComplete.Tick += (sender, e) => TimerGetStatusComplete_Tick(DTO.NotificationForm);
            DTO.NotificationForm.Owner = formOwner;
            DTO.NotificationForm.ShowDialog();


        }
        private void TimerGetStatusComplete_Tick(frmNotification _frm)
        {
            frmNotification frm = _frm;
            int status = ContestBUS.Instance.GetStatusDivisionShift(frm.DivisionShiftID, Sql);
            int statusContestant = ContestantBUS.Instance.GetStatusContestantShift(frm.ContestantShiftID);
            if (status == Constant.STATUS_DIVISION_COMPLETE || statusContestant == Constant.STATUS_SIGNED)
            {
                frm.SetMbtnOK(true);
            }
        }
        ...
        public void HandleRichTextBoxStyle1(RichTextBox rtb)
        {
            rtb.ReadOnly = true;
            rtb.BackColor = Constant.COLOR_WHITE;
        ...
        public bool HandleCheckDB(out ErrorController error)
        {
            ...
            try
            {
                ...
                if (sql.IsConnection)
                {
                    error = new ErrorController(1, "OK");
                    return true;
                }
                else
                {
                    error = new ErrorController(1, "false");
            ...
            catch (SqlException ex)
            {
                ...
                error = new ErrorController(2, errorMessages.ToString());
                return false;
            }
        }
